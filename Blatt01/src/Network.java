...
public class Network {
    ...
    public Network(int inter, int receptors, int cortical) {
        ...
        if (receptors < 3 || inter < receptors) {
            throw new RuntimeException("Invalid neuron numbers");
        }
        ...
        for (int i = 0; i < receptors; i++) {
            c_idx = i % 3;
            this.neurons[i] = new Photoreceptor(i, this.receptortypes[c_idx]);
        }
        // init inter
    ...
    public void addSynapse(Neuron n1, Neuron n2) {
        // TODO
        Synapse syn_tmp = new Synapse(n1, n2);
        n1.addSynapse(syn_tmp);
    }
    ...
    public double[] signalprocessing(double[] input) {
        ...
        // TODO
        int neuron_num = this.neurons.length;
        //System.out.println("cortical "+this.cortical);

        for (int i = 0; i < this.receptors; i++) {
            Photoreceptor neuron;
            neuron = (Photoreceptor) (this.neurons[i]);

            neuron.integrateSignal(input);

        }
        for (int j = neuron_num - this.cortical; j < neuron_num; j++) {
            CorticalNeuron cortical_tmp = (CorticalNeuron) this.neurons[j];
            double[] cortical_out = cortical_tmp.getSignal();
            for (int k = 0; k < 3; k++) {
                signal[k] = cortical_out[k];
            }
        }
...
//        // add Synapse
        ...
        for (int i = receptors; i < inter + receptors; i++) {
            //TODO
            neural.addSynapse(neural.neurons[i - receptors], neural.neurons[i]);
        }
        ...
        for (int i = inter + receptors; i < inter + receptors + cortical; i++) {
            ...
            for (int j = receptors; j < inter + receptors; j++) {
                neural.addSynapse(neural.neurons[j], neural.neurons[i]);
            }
        }
